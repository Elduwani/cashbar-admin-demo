...
export default function Index() {
   ...
   const dataSet: DataSet[] = [
      ...
      { label: "revenue", color: "text-indigo-600", data: chartData.data, circle: true },
   ]

   const data = dataSet.map(set => getChartData(set.data, timePeriod.value))

   return (
      <div className="w-full h-full flex bg-slate-50">
         <div className={`h-full w-full max-w-sm p-8 bg-slate-100`}>
            <HomeAggregate />
         </div>
         <div className="w-full overflow-y-auto scrollbar p-8 space-y-6">
            ...
            <div className="max-w-xs">
               <Button goto="/transactions" variant="teal">See all transactions</Button>
            </div>
            <div className="flex">
